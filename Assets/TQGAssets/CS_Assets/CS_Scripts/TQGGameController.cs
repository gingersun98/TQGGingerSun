			...
			if ( isGameOver == false )
			{
				...
				if ( questionIsUsed == false )
				{
					...
					if ( currentQuestion < questions.Length )
					{
						...
						// 		rawImage.texture = questions[currentQuestion].video.texture;
								
						// 		// Set the video to loop forever
						// 		questions[currentQuestion].video.isLooping = true;
		...
		public void LoadXml( string xmlPath, bool append )
		{
			...
			foreach ( XmlNode XmlRecord in xmlRecords )
			{
				...
				foreach ( XmlNode XmlQuestion in XmlQuestions )
				{
					...
					// 	questionsTemp[questionIndex].video = Resources.Load<UnityEngine.Video.VideoClip>("Videos/" + XmlQuestion.InnerText); 
					// 	#else
